...
function getToken(): string {
  ...
  throw new Error(
    "X_BEARER_TOKEN not set. Get a bearer token from https://developer.x.com and export X_BEARER_TOKEN=your-token"
  );
}
...
async function apiGet(url: string): Promise<RawResponse> {
  ...
  if (res.status === 401 || res.status === 403) {
    throw new Error(
      `X API ${res.status}: Invalid or expired X_BEARER_TOKEN. Generate a new token at https://developer.x.com`
    );
  }
  ...
  if (res.status === 429) {
    ...
    const resetInfo = reset
      ? `Resets at ${new Date(parseInt(reset) * 1000).toISOString()} (${Math.max(parseInt(reset) - Math.floor(Date.now() / 1000), 1)}s)`
      : "Try again later";
    throw new Error(`X API rate limited (429). ${resetInfo}`);
  }
...
export async function thread(
  ...
  // Fetch the root tweet separately (single tweet lookup returns data as object, not array)
  try {
    const rootUrl = `${BASE}/tweets/${conversationId}?${FIELDS}`;
    const raw = await apiGet(rootUrl);

    // Single tweet lookup returns { data: {...}, includes: {...} }
    // Need to wrap data in array for parseTweets
    if (raw.data && !Array.isArray(raw.data)) {
      const rootTweets = parseTweets({ ...raw, data: [raw.data] });
      if (rootTweets.length > 0) {
        // Only prepend if not already in thread results
        const rootId = rootTweets[0].id;
        if (!tweets.some((t) => t.id === rootId)) {
