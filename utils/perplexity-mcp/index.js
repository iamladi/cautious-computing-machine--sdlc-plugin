...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const { query, recency = 'month', frequency_penalty, max_tokens, model = defaultModel, presence_penalty, return_citations = true, return_images = false, stream = false, temperature, top_k, top_p } = args;
    ...
    if (frequency_penalty !== undefined)
        apiRequest.frequency_penalty = frequency_penalty;
    if (max_tokens !== undefined)
        apiRequest.max_tokens = max_tokens;
    if (presence_penalty !== undefined)
        apiRequest.presence_penalty = presence_penalty;
    if (return_citations !== undefined)
        apiRequest.return_citations = return_citations;
    if (return_images !== undefined)
        apiRequest.return_images = return_images;
    if (stream !== undefined)
        apiRequest.stream = stream;
    if (temperature !== undefined)
        apiRequest.temperature = temperature;
    if (top_k !== undefined)
        apiRequest.top_k = top_k;
    if (top_p !== undefined)
        apiRequest.top_p = top_p;
    try {
